...
public class ContactsManager extends ContentObserver {
	...
	private synchronized void fetchContactsSync() {
		...
		Log.i(TAG, "[ContactsManager] Step 0 for " + contacts.size() + " contacts: " + time + " elapsed since starting");
		...
		if (hasContactsAccess()) {
			...
			if (isOrgVisible) {
				...
				if (c != null) {
					while (c.moveToNext()) {
				    	String id = c.getString(c.getColumnIndex(Data.CONTACT_ID));
				    	String org = c.getString(c.getColumnIndex(CommonDataKinds.Organization.COMPANY));
				    	LinphoneContact contact = androidContactsCache.get(id);
				    	if (contact != null) {
			...
			time = String.format("%02d:%02d",
				    ...
				    TimeUnit.MILLISECONDS.toSeconds(timeElapsed) -
				    TimeUnit.MINUTES.toSeconds(TimeUnit.MILLISECONDS.toMinutes(timeElapsed)));
			Log.i(TAG, "[ContactsManager] Step 1 for " + contacts.size() + " contacts: " + time + " elapsed since starting");

			c = getPhonesCursor(contentResolver);
			if (c != null) {
				while (c.moveToNext()) {
					String id = c.getString(c.getColumnIndex(CommonDataKinds.Phone.CONTACT_ID));
			...
			if (c != null) {
				while (c.moveToNext()) {
					String id = c.getString(c.getColumnIndex(Data.CONTACT_ID));
			    	String sip = c.getString(c.getColumnIndex(CommonDataKinds.SipAddress.SIP_ADDRESS));
			    	LinphoneContact contact = androidContactsCache.get(id);
			    	if (contact != null) {
			...
			time = String.format("%02d:%02d",
				    ...
				    TimeUnit.MILLISECONDS.toSeconds(timeElapsed) -
				    TimeUnit.MINUTES.toSeconds(TimeUnit.MILLISECONDS.toMinutes(timeElapsed)));
			Log.i(TAG, "[ContactsManager] Step 2 for " + contacts.size() + " contacts: " + time + " elapsed since starting");
			...
			time = String.format("%02d:%02d",
				    ...
				    TimeUnit.MILLISECONDS.toSeconds(timeElapsed) -
				    TimeUnit.MINUTES.toSeconds(TimeUnit.MILLISECONDS.toMinutes(timeElapsed)));
			Log.i(TAG, "[ContactsManager] Step 3 for " + contacts.size() + " contacts: " + time + " elapsed since starting");

			androidContactsCache.clear();
		} else {
			Log.w(TAG, "[Permission] Read contacts permission wasn't granted, only fetch LinphoneFriends");
		}
	...
	private Cursor getPhonesCursor(ContentResolver cr) {
		Cursor cursor = cr.query(CommonDataKinds.Phone.CONTENT_URI,
			     new String[] { CommonDataKinds.Phone.NUMBER, CommonDataKinds.Phone.CONTACT_ID },
			     null, null, CommonDataKinds.Phone.CONTACT_ID + " ASC");
		return cursor;
	}

	private Cursor getSipCursor(ContentResolver cr) {
		String select = Data.MIMETYPE + "=?";
		String[] projection = new String[] { Data.CONTACT_ID, CommonDataKinds.SipAddress.SIP_ADDRESS };
		Cursor c = cr.query(Data.CONTENT_URI, projection, select, new String[]{ CommonDataKinds.SipAddress.CONTENT_ITEM_TYPE }, null);
		return c;
	}

	private Cursor getOrganizationCursor(ContentResolver cr) {
		String select = Data.MIMETYPE + "=?";
		String[] projection = new String[] { Data.CONTACT_ID, CommonDataKinds.Organization.COMPANY };
		Cursor c = cr.query(Data.CONTENT_URI, projection, select, new String[]{ CommonDataKinds.Organization.CONTENT_ITEM_TYPE }, null);
		return c;
	}
}
